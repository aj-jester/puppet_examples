...
def check_oplog(con, warning, critical, perf_data):
    ...
    try:
        ol_time = replication_get_time_diff(con) / 3600.0
        message = "Oplog time " + "%.1f" % ol_time + " hrs"

        approx_level = ol_time
        message += performance_data(perf_data, [("%.1f" % ol_time, 'oplog_time', warning, critical)])
